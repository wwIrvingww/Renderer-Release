...
pub fn triangle(v1: &Vertex, v2: &Vertex, v3: &Vertex) -> Vec<Fragment> {
  ...
  let light_dir = Vec3::new(0.0, 0.0, 1.0);  // Dirección de la luz
  ...
  for y in min_y..=max_y {
    for x in min_x..=max_x {
      ...
      if w1 >= 0.0 && w1 <= 1.0 && w2 >= 0.0 && w2 <= 1.0 && w3 >= 0.0 && w3 <= 1.0 {
        ...
        let intensity = dot(&normal, &light_dir).max(0.0); // Calcula la intensidad de la luz
        ...
        fragments.push(Fragment::new(
          ...
          depth, 
          normal, 
          intensity, // Pasa la intensidad de la luz
